import { Session } from 'server/sessions';
import Avatar from '../Reusable/Avatar';
import DirectMessagePopUp from './DirectMessagePopUp';
import { useState } from 'react';
import classNames from 'classnames';

interface Props {
  userId: string;
  users: Partial<Session>[];
  isMobile: boolean;
  sendDirectMessage: (to: string, message: string) => void;
}

export default function UserBar({
  users,
  isMobile,
  sendDirectMessage,
  userId,
}: Props) {
  const [openModalUserId, setOpenModalUserId] = useState<string | null>(null);
  const handleDm = (to: string) => (message: string) => {
    setOpenModalUserId(null);
    sendDirectMessage(to, message);
  };
  const baseUserBarStyles = `max-h-full flex flex-col md:flex-[1] md:items-start items-center flex-[0.5] min-w-14 bg-discord-black border-l-1 border-discord-dark overflow-y-auto gap-2`;
  const userBarStyles = classNames(
    baseUserBarStyles,
    isMobile ? 'max-w-20' : 'max-2-120'
  );
  return (
    <div className={userBarStyles}>
      {users.map(u => {
        return (
          u.userId !== userId && (
            <div
              className="bg-discord-black hover:bg-discord-dark w-full rounded pt-1 pb-1 pl-3 pr-1"
              key={u.userId}
              onClick={() => setOpenModalUserId(u.userId!)}
            >
              <Avatar
                avatar={u.avatar}
                isConnected={u.connected || false}
                showStatus={!isMobile ? true : false}
                showName={!isMobile ? true : false}
                name={u.username}
                showIndicator={true}
              ></Avatar>
              <DirectMessagePopUp
                isOpen={openModalUserId === u.userId}
                isMobile={isMobile}
                onSubmit={handleDm(u.userId!)}
                onClose={() => setOpenModalUserId(null)}
              ></DirectMessagePopUp>
            </div>
          )
        );
      })}
    </div>
  );
}
